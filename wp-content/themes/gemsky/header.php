...
<div id="gemsky-content" class="gemsky-content">
    <nav id="nav" class="ui inverted menu gemsky-nav">
        ...
        <div id="mobileMenu" class="mobileMenu">
	        <?php
	        $defaults = array(
		        'theme_location'  => 'mobile_menu',//用于在调用导航菜单时指定注册过的某一个导航菜单位置，如果没有指定，则显示第一个。
		        ...
		        'container_class' => '',//ul 父节点的 class 属性值。
		        'container_id'    => 'mobileMenu_container',//ul 父节点的 id 属性值。
		        'menu_class'      => 'ui styled accordion',//ul 节点的 class 属性值。
		        'menu_id'         => 'mobileMenu_ul',//默认值: menu slug, 自增长的。ul 节点的 id 属性值。
		        ...
		        'walker'          => new gemsky_mobilemenu_walker() //自定义的遍历对象，调用一个对象定义显示导航菜单。
        ...
        <div class="right menu">
			...
			$defaults = array(
				...
				//			'items_wrap'      => '<ul id="%1$s" class="%2$s">%3$s</ul>',//使用字符串替换修改ul的class。
				'depth'           => 2, //显示菜单的深度, 当数值为 0 时显示所有深度的菜单。
				'walker'          => new gemsky_topmenu_walker() //自定义的遍历对象，调用一个对象定义显示导航菜单。
				//若无参数这调用 Walker_Nav_Menu类的walker,
				//若自定义只需模仿Walker_Nav_Menu继承Walker
			);
			//		$default = array(
...
<!--                </div>-->
<!--            </div>-->
            <div id="loginOrReg" class="item loginOrReg">
	            <?php wp_register(); ?>
	            <?php wp_loginout(); ?>
            </div>
        </div>
